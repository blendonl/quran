...
class QuranPhonemeEngine:
    def __init__(
        self,
        model_name: str = settings.quran_phoneme_model,
    ...
    def _load_model(self):
        ...
        logger.info(
            "Loading Quran phoneme model: %s (device=%s, dtype=%s)",
            model_path, self.device, self.dtype,
        )
        ...
        self.model.eval()
        logger.info("Quran phoneme model loaded")
    ...
    def _decode(self, logits: torch.Tensor) -> PhonemeResult | None:
        ...
        logger.info("QuranPhoneme: '%s' (avg_conf=%.3f, n=%d)", raw_ipa, avg_conf, len(phonemes))
